...
uint32_t play_song(unsigned char * song, uint32_t size){
	...
	int index = 0;
	if(notes[index].delay == 0 && notes[index + 1].delay == 0 && notes[index + 2].delay == 0){
		while(index < note_incre){
			...
			add_tone(notes[index].note, notes[index].velocity);
			delay_counter = 0;
			while(delay_counter < (time + notes[index - 2].delay + notes[index - 1].delay + notes[index].delay)){
				play_tones();
			}
			remove_tone(notes[index - 2].note);
			remove_tone(notes[index - 1].note);
